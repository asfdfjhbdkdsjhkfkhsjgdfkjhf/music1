...
//by ,$ ReBeL ء , 🔕#4777 'CODES SERVER'
	...
	.setDescription(`:notes: الان يتم تشغيل : **${serverQueue.songs[0].title}**`)
		...
		if (!msg.member.voiceChannel) return msg.channel.send('أنت لست بروم صوتي .');
        serverQueuel.voiceChannel.join().then(message.channel.send(':ok:'));
...
const adminprefix = "1vip";//by ,$ ReBeL ء , 🔕#4777 'CODES SERVER'
const devs = ['460389855382470662'];//by ,$ ReBeL ء , 🔕#4777 'CODES SERVER'
...
${prefix}join ⇏ البوت يدخل الروم
**
 `)//by ,$ ReBeL ء , 🔕#4777 'CODES SERVER'
   message.author.sendEmbed(embed)//by ,$ ReBeL ء , 🔕#4777 'CODES SERVER'
    
   }
   }); 
     client.on("message", message => {
 if (message.content === `${prefix}help`) {
	 message.reply('شف خاصك يالعلو');
   }
